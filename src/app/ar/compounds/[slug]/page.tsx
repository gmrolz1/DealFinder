...
import { campaignWaTextCompound } from "@/lib/campaign";
...
export default async function CompoundPageAr({
  ...
  const waHref = goHref({
    channel: "wa",
    locale: "ar",
    text: campaignWaTextCompound(name, areaName, priceLabel, "ar"),
  });
  const callGoHref = goHref({ channel: "tel", locale: "ar" });
  const propertyTypes =
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 pb-28 pt-7 sm:px-6">
      ...
      <StickyContact locale="ar" waHref={waHref} callHref={callGoHref} />
    </div>
  );
}
