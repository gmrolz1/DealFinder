...
import { type Locale, t, tpl, localizedPath } from "@/lib/i18n";

export function DevelopersIndex({ locale }: { locale: Locale }) {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      <h1 className="text-[32px] font-extrabold uppercase tracking-tight text-ink sm:text-[44px]">
        {t("index.heading.dev", locale)}
      </h1>
      <p className="mt-1 text-[12px] font-semibold uppercase tracking-[0.08em] text-taupe">
        {tpl("index.subtitle.dev.tpl", locale, {
          a: formatNumber(developers.length),
          b: formatNumber(withInventory),
        })}
      </p>
      ...
      <div className="mt-6 grid grid-cols-1 border-l border-t border-data sm:grid-cols-2 lg:grid-cols-3">
        {developers.map((d) => {
          ...
          return (
            <Link
              key={d.nawy_id}
              href={localizedPath(`/developers/${d.slug}`, locale)}
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className="text-[10px] font-semibold uppercase tracking-[0.07em] text-taupe">
                  {d.available > 0
                    ? tpl("index.cardAvailable.tpl", locale, {
                        n: formatNumber(d.available),
                      })
                    : t("index.cardEmpty", locale)}
                </p>
              </div>
            </Link>
          );
